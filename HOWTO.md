...
The most up-to date version of this document is available at:
    ...
    https://github.com/Squarebitdata/Sbit-ElectrumServer/blob/master/HOWTO.md
...
commands starting with a dollar should be run as a normal user. We assume
the user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run SquareBitd and keep a copy of the full blockchain, 
which is roughly 4 GB in April 2014. If you have less than 2 GB of RAM 
make sure you limit SquareBitd to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running SquarerBitd and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running SquareBitd and Electrum.
...
If you don't see `/home/username/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download SquareBitd

Older versions of Electrum used to require a patched version of SquareBitd. 
This is not the case anymore since SquareBitd supports the 'txindex' option.

If you prefer to compile SquareBitd, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libminiupnpc-dev
    $ sudo su - username
    $ cd ~/src && git clone https://github.com/Squarebitdata/sbit.git
    $ cd Sbit/src
    $ make -f makefile.unix
    $ strip SquareBitd
    $ cp -a ~/src/Sbit/src/SquareBitd ~/bin/SquareBitd

### Step 3. Configure and start SquareBitd

In order to allow Electrum to "talk" to SquareBitd, we need to set up an RPC
username and password for SquareBitd. We will then start SquareBitd and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.SquareBit
    $ $EDITOR ~/.SquareBit/SquareBit.conf

Write this in `SquareBit.conf`:
...
If you have an existing installation of SquareBitd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ SquareBitd -reindex

If you have a fresh copy of SquareBitd start `SquareBitd`:

    $ SquareBitd

Allow some time to pass, so `SquareBitd` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ SquareBitd getinfo

Before starting the electrum server your SquareBitd should have processed all
blockes and caught up to the current height of the network.
You should also set up your system to automatically start SquareBitd at boot
time, running as the user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/Squarebitdata/Sbit-ElectrumServer.git
    $ cd Sbit-ElectrumServer
    $ sudo configure
...
and further instructions from the Electrum-Sbit full archival server foundry at:
http://foundry.electrum-sbit.org/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-sbit.conf) when starting up. This
file includes the database setup, SquareBitd RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-sbit.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-sbit-server.
If you're using a different user to run electrum and have added it manually like shown in this HOWTO run 
...
1. To increase security you may want to close SquareBitd for incoming connections and connect outbound only

2. Consider restarting SquareBitd (together with electrum-sbit-server) on a weekly basis to clear out unconfirmed
...
Note: If you want to run the server without installing it on your system, just run 'run_electrum_sbit_server" as the
unprivileged user.
...
If your system supports it, you may add electrum-sbit-server to the /etc/init.d directory. 
...
SquareBit coins to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-sbit and we'll try to congratulate you
on supporting the community by running an Electrum-Sbit node.

If you're operating a public Electrum-Sbit server please subscribe
to the following mailing list:
https://groups.google.com/forum/#!forum/electrum-sbit-server
It'll contain announcements about important updates to Electrum-Sbit
